...
const ytdlBinary = (() => {
  if(process.env.ytdlBinary)
    return process.env.ytdlBinary;
...
function youtubeDl(args, options, callback) {
  ...
  let _bin = ytdlBinary
  let _args = args
  let _options = { ...execFileOpts, ...options}
  if(process.env.ytdlModuleCwd) {
    _bin = 'python'
    _args = ['-m','youtube_dl'].concat(args)
    _options.cwd = process.env.ytdlModuleCwd
  }
  execFile(_bin, _args, _options, function done(err, stdout, stderr) {
    if (err) {
...
ytdl.getInfo = function getInfo(url, args, options, callback) {
  ...
  return new Promise((resolve, reject) => {
      call(url, defaultArgs, args, options, function done(err, data) {
        ...
        return callback ? callback(null, info.length === 1 ? info[0] : info) : resolve(info.length === 1 ? info[0] : info)
    });
  });
};
 ...
 * @param {String} url 
 * @param {Array.<String>} args `youtube-dl`arguments
 * @param {Object} options `child_process.spawn` options
 * <code>
 * let dl = ytdl.spawn("https://www.youtube.com/watch?v=grXmSw1WqMk");
 * 
 * dl.stdout.on('data', function(data) {
 *   sails.sockets.broadcast(req.socket.id,'prepare-file',data);
 * });
 * dl.stderr.on('data', function(data) {
 *   sails.sockets.broadcast(req.socket.id,'prepare-file-error',data);
 * });
 * 
 * dl.on('close', function(exitCode) {
 *   sails.sockets.broadcast(req.socket.id,'display-player', {exitCode: exitCode});
 * });
 * 
 * req.socket.on("disconnect", function(a, b, c, d) {
 *   dl.kill();
 * });
 * </code>
 */
ytdl.spawn = function(url, args, options) {
  args = args || [];
  if(url)
    args.push(url);
  options = options || {};
  ...
  return spawn(_bin, _args, _options);
}
